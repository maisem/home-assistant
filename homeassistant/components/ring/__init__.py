...
async def async_setup_entry(hass, entry):
    ...
    async def refresh_all(_):
        ...
        await asyncio.gather(
            *(
                [
                    info["device_data"].update_all()
                    for info in hass.data[DOMAIN].values()
                ]
                + [
                    info["dings_data"].update_all()
                    for info in hass.data[DOMAIN].values()
                ]
            )
        )
...
class GlobalDataUpdater:
    ...
    def add_listener(self, update_callback):
        ...
        if not self.listeners:
            self._unsub_interval = async_track_time_interval(
                self.hass, self.update_all, self.update_interval
            )
    ...
    async def update_all(self, _now: Optional[int] = None) -> None:
        """Time to update."""
        if not self.listeners:
            return
